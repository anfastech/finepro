  ...
  const renderProject = (project: Project, isNested = false) => {
    ...
    return (
      <Link href={href} key={project.$id}>
        <div
          className={cn(
            "flex items-center gap-2.5 p-2.5 rounded-md transition cursor-pointer text-neutral-300 hover:text-white hover:bg-white/10",
        ...
        return (
          <div key={folder.id} className="flex flex-col">
            <button
              ...
              className={cn(
                "flex items-center gap-2 p-2.5 rounded-md transition text-neutral-300 hover:text-white hover:bg-white/10 text-left"
              )}
            >
              {isExpanded ? (
